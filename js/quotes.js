const quotes = [
	...
	{text:"행복의 문이 하나 닫히면 다른 문이 열린다 그러나 우리는 종종 닫힌 문을 멍하니 바라보다가 우리를 향해 열린 문을 보지 못하게 된다",  author:"헬렌켈러"},
	...
	{text:"행복한 삶을 살기위해 필요한 것은 거의 없다.", author:"마르쿠스 아우렐리우스 안토니우스"},
	...
	{text:"너무 소심하고 까다롭게 자신의 행동을 고민하지 말라 . 모든 인생은 실험이다 . 더많이 실험할수록 더나아진다", author:"랄프 왈도 에머슨"},
	{text:"한번의 실패와 영원한 실패를 혼동하지 마라", author:"F.스콧 핏제랄드"},
	{text:"내일은 내일의 태양이 뜬다", author:"무명씨"}
]
...
const qutoeList = document.querySelector("#qutoe_list");
const qutoeText = qutoeList.querySelector("#quote_text");
const qutoeAuthor = qutoeList.querySelector("#quote_author");

const quoteObj = quotes[Math.floor(Math.random() * quotes.length)];

qutoeText.innerText = quoteObj.text;
qutoeAuthor.innerText = quoteObj.author;
